...
namespace Crimenuts.Core.Game.Pack.Worlds
{
    public abstract class BaseWorld : IWorld
    {
        ...
        protected Person.Person CreatePerson( ProfileType type, string name )
        {
            var profile = Profiles[ ( int ) type ];
            return new Person.Person( profile ) { Name = name };
        }

        protected Person.Person NormalPerson( string name )
        {
