...
namespace Papagames.Detective.App.Console
{
    internal partial class Program
    {
        ...
        private static void RunStatistics(Game game)
        {
            ...
            {
                for (var m = 1; m <= game.CalcMaxMurdersNum(n); m++)
                {
                    var stat = CalcGameSuccessRate(count, game, n, m);
        ...
        private static Tuple<double, double, double> CalcGameSuccessRate(int count, Game game, int n, int m)
        {
            var success = 0.0;
            var sd = 0.0;
            var ad = 0.0;
        ...
        private static Tuple<bool,int> RunGame(Game game, int memberNum, int murderNum)
        {
            game.Run(memberNum, murderNum);
            return new Tuple<bool, int>(game.DidDeteciveWin, game.CurrentDay);
        }
    }
}
